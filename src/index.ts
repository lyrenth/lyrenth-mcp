...
async function main(): Promise<void> {
  const server = new McpServer({ name: "lyrenth", version: "0.1.6" });
  ...
  registerTools(server, {
    ...
    // Same gate as src/http.ts, off by default: search is built and dark,
    // and the stdio build is what every npx and .mcpb user runs. This was
    // hardcoded true and shipped the search tool to a product that does
    // not serve it; scripts/check-tools.mjs now fails the build on that.
    includeSearch: process.env.LYRENTH_MCP_SEARCH === "1",
  });
